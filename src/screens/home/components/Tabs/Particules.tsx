...
import Animated, { add, and, divide, interpolate, multiply, neq } from 'react-native-reanimated';
    ...
    <View style={styles.container} pointerEvents="none">
      <View style={styles.particules}>
        {topParticules.map(particule => {
          ...
          const translateX = subParticules.reduce(acc => withSpringTransition(acc), x);
          const translateY = subParticules.reduce(acc => withSpringTransition(acc), top);
          const scale = subParticules.reduce(acc => withSpringTransition(acc), s);
          return (
            <Animated.View
              key={particule}
              style={[
