class Api::V1::FixedEmissionsController < Api::V1::ApiBaseController
  ...
  def create
    ...
    if @fixed_emission.save
      Emission.create!(amount: @fixed_emission.monthly_emitted_carbon, emissionable: @fixed_emission, user_id: current_user.id)
  ...
  private
    ...
    def fixed_emission_params
      params.require(:fixed_emission).permit(:user_id, :house_surface, :electricity_consumption, :gas_consumption, :wood_consumption, :wood_type, :fuel_consumption, :roommates, :clothes, :furnitures, :others, :breakfasts_per_week, :red_meats_per_week, :vegan_per_week, :vegetarian_per_week, :white_meats_per_week)
    end
end
